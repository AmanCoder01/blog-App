...
import { getPostApi, updatePostApi } from "../services/apiCall"
...
const UpdatePost = () => {
    ...
    useEffect(() => {
        const fetchPost = async () => {
            try {
                ...
                const res = await getPostApi(`postId=${postId}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await updatePostApi(`${formData._id}/${currentUser._id}`, formData);
              console.log(res);
            if (res.status === 200) {
